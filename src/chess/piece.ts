import {Pair, equals, dedup, hash, unhash, NotImplementedError, PAWN_HOME_RANK, Color, MoveType} from './const.js';
...
export class Square {
    ...
    toString(): string {
        if (this.occupant) {return `[${this.occupant.toString()}]`}
        return '[ ]';
    }
    ...
    render(): HTMLElement {
        ...
        result.onclick = (e) => {
            ...
            if (SELECTED_PIECE) {
                if (SELECTED_PIECE.legalMoves().map(hash).includes(hash({row: this.row, col: this.col}))) {
                    SELECTED_PIECE.game!.attemptMove(SELECTED_PIECE.color, SELECTED_PIECE, this);
                }
...
class Leaper extends Piece {
    ...
    legalMoves(): Pair[] {
        ...
        let targets = this.moves.flatMap(
            move => [-1, 1].map(sign => move.row * sign).flatMap(
                first => [-1, 1].flatMap(sign => [{
                    row: first,
                    col: move.col * sign,
                }, {
                    row: move.col * sign,
        ...
        ).map(relative => ({row: relative.row + square.row, col: relative.col + square.col}));
...
class Rider extends Piece {
    ...
    legalMoves(): Pair[] {
        ...
        const directions = dedup(this.moves.flatMap(
            move => [-1, 1].map(sign => move.row * sign).flatMap(
                first => [-1, 1].flatMap(sign => [{
                    row: first,
                    col: move.col * sign,
    ...
    ride(row: number, col: number): Pair[] {
        ...
        const targets: Pair[] = [];
        let square = this.state.getSquare(this.square.row + row, this.square.col + col);
...
class Pawn extends Piece {
    ...
    legalMoves(): Pair[] {
        ...
        const {game, state, square: {row, col}} = this;
        // normal move. 2 step. capture, en passant
        const yDir = this.color === Color.WHITE ? -1 : 1;
        let targets = [{row: row + yDir, col: col}];

        // 2 move
        if ((this.square.row === PAWN_HOME_RANK && this.color === Color.BLACK) ||
            (this.square.row === state.ranks - PAWN_HOME_RANK - 1 && this.color === Color.WHITE)) {
        ...
        // capture
        const targetSquares = [-1, 1].map(xDir => state.getSquare(row + yDir, col + xDir));
...
export class Game {
    ...
    attemptMove(mover: Color, piece: Piece, target: Square) {
        const legalMove = piece.legalMoves().find(location =>
            equals(location, target));
        if (!legalMove) {
        ...
        const end = target;
        const isCapture = !!(target.occupant && target.occupant.color !== piece.color);
        ...
        const move = {before, after, piece, start, end, isCapture, captured, color, type: 'move'};
...
export class BoardState {
    ...
    isInCheck(color: Color): boolean {
        const opposingPieces = this.squares.flat().map(square => square.occupant).filter(occupant => !!occupant && occupant.color !== color);
        const opposingControlledSquares = dedup(opposingPieces.flatMap(piece => piece!.legalMoves()));
        const ownRoyalSquares = this.squares.flat().filter(square => !!square.occupant && square.occupant.color === color && square.occupant.isRoyal);

        return ownRoyalSquares.some(square => opposingControlledSquares.map(hash).includes(hash(square)));
    }
